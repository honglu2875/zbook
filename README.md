...
- long cells can be height-limited with a double-click on their gutter, while source and output can be collapsed independently; these view preferences persist per workspace without changing the notebook file;
...
- Codex gets dedicated read/apply cell tools: notebook edits go directly through React, are revision-checked, briefly lock the editor, apply atomically, and save without a shell/edit/refresh round trip; source-light reads plus `move_after` and `swap` operations make cell reordering inexpensive;
